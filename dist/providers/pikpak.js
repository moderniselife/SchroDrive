...
class PikPakProvider {
     ...
     */
    isConfigured() {
        return !!(config_1.config.pikpakUsername && config_1.config.pikpakPassword);
    }
    /**
    ...
    async listTorrents() {
        ...
        try {
            ...
            rateLimiter_1.rateLimiter.setCache(TORRENT_LIST_CACHE_KEY, tasks);
    ...
    async getInfoHash(torrentId) {
        ...
        const cached = rateLimiter_1.rateLimiter.getCache(TORRENT_LIST_CACHE_KEY);
        if (cached) {
            const task = cached.find((t) => String(t.id) === String(torrentId));
            if (task?.hash)
                return task.hash;
            if (task?.hashString)
        ...
        try {
            ...
            const tasks = Array.isArray(data?.tasks) ? data.tasks : [];
            const task = tasks.find((t) => String(t.id) === String(torrentId));
    ...
    async resolveDownloadUrl(torrentId, fileId, _linkIndex) {
        const downloadToken = tokenRotator_1.tokenRotator.getDownloadToken(PROVIDER_NAME);
     ...
     */
    hasDirectWebDAV() {
        return !!(config_1.config.pikpakWebdavUrl && config_1.config.pikpakWebdavUsername && config_1.config.pikpakWebdavPassword);
    }
    /**
    ...
    getWebDAVConfig() {
        if (!this.hasDirectWebDAV())
            return null;
        return {
            url: config_1.config.pikpakWebdavUrl,
            username: config_1.config.pikpakWebdavUsername,
            password: config_1.config.pikpakWebdavPassword,
        };
    }
    /**
     ...
     */
    getBridgePort() {
        return config_1.config.webdavBridgePortPIKPAK;
    }
    // -------------------------------------------------------------------------
    // Private Helpers
    // -------------------------------------------------------------------------
...
exports.PikPakProvider = PikPakProvider;
// ===========================================================================
// Self-Registration
// ===========================================================================
registry_1.registry.register(new PikPakProvider());
// Register with token rotator for download token cycling
tokenRotator_1.tokenRotator.registerProvider(PROVIDER_NAME, config_1.config.pikpakUsername, config_1.config.pikpakDownloadTokens);
