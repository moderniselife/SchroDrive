...
const DEFAULT_STORE_PATH = (0, path_1.join)(process.env.CONFIG_DIR || process.env.MOUNT_BASE || "/config", "rate-limit-store.json");
...
class RateLimitStore {
    ...
    load() {
        try {
            if ((0, fs_1.existsSync)(STORE_PATH)) {
                const raw = (0, fs_1.readFileSync)(STORE_PATH, "utf-8");
    ...
    save() {
        try {
            const dir = (0, path_1.dirname)(STORE_PATH);
            if (!(0, fs_1.existsSync)(dir)) {
                (0, fs_1.mkdirSync)(dir, { recursive: true });
            }
            this.data.lastSaved = new Date().toISOString();
            (0, fs_1.writeFileSync)(STORE_PATH, JSON.stringify(this.data, null, 2), "utf-8");
        }
        catch (err) {
     ...
     */
    recordSuccess(provider, endpoint, responseTimeMs) {
        ...
        // Recalculate average
        stats.avgResponseMs = Math.round(stats.recentResponseTimesMs.reduce((a, b) => a + b, 0) / stats.recentResponseTimesMs.length);
        ...
        if (timeSinceRateLimit > 5 * 60 * 1000) {
            stats.optimalDelayMs = Math.max(MIN_DELAY_FLOOR_MS, Math.round(stats.optimalDelayMs * 0.95) // Decrease by 5%
            );
        }
        this.dirty = true;
    }
     ...
     */
    recordRateLimit(provider, endpoint, retryAfterS) {
        const stats = this.getEndpoint(provider, endpoint);
        stats.totalRequests++;
        ...
        if (retryAfterS != null && retryAfterS > 0) {
            ...
            stats.optimalDelayMs = Math.min(MAX_DELAY_CAP_MS, Math.round((retryAfterS * 1000) * 1.2));
        }
        else {
            // No Retry-After — double the current delay
            stats.optimalDelayMs = Math.min(MAX_DELAY_CAP_MS, stats.optimalDelayMs * 2);
        }
        // Update provider-level delay to the max of all endpoints for that provider
        this.updateProviderDelay(provider);
        this.dirty = true;
        console.log(`[${new Date().toISOString()}][rate-store] ${provider}:${endpoint} ` +
     ...
     */
    getEndpointDelay(provider, endpoint) {
        const k = this.key(provider, endpoint);
    ...
    getProviderStats(provider) {
        return Object.values(this.data.endpoints).filter((s) => s.provider === provider);
    }
    /**
...
exports.rateLimitStore = new RateLimitStore();
