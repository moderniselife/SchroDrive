...
exports.MEDIA_VIEWS = void 0;
exports.classifyTorrent = classifyTorrent;
exports.isMediaView = isMediaView;
exports.filterByCategory = filterByCategory;
exports.onlyBiggestFile = onlyBiggestFile;
// ===========================================================================
 ...
 */
const ANIME_HEURISTIC_PATTERNS = [
 ...
 */
function looksLikeAnime(name) {
    // Primary check: CRC hash in brackets
    if (/\[[a-fA-F0-9]{8}\]/i.test(name))
        return true;
 ...
 */
function matchesFilter(torrentName, fileNames, filter) {
    switch (filter.type) {
        case 'regex':
        ...
        case 'any_file_inside_regex':
            ...
            if (filter.pattern.test(torrentName))
                return true;
 ...
 * ```
 */
function classifyTorrent(torrentName, fileNames) {
    // Enhanced anime detection: use our heuristic detector first,
    // then fall back to Zurg's regex-only approach
    if (looksLikeAnime(torrentName))
        return 'anime';
    if (fileNames?.some((fn) => looksLikeAnime(fn)))
        return 'anime';
 ...
 */
exports.MEDIA_VIEWS = ['__all__', 'anime', 'shows', 'movies'];
/**
 ...
 */
function isMediaView(name) {
    return exports.MEDIA_VIEWS.includes(name);
}
/**
 ...
 */
function filterByCategory(torrents, category) {
 ...
 */
function onlyBiggestFile(files) {
    if (files.length <= 1)
        return files;
    let biggest = files[0];
