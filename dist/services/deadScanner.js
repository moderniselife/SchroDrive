 ...
 */
async function tryReaddViaIndexer(title, excludeProvider) {
    const results = await (0, index_1.searchIndexer)(title);
    // Filter out blacklisted results before picking
    const filteredResults = results.filter((r) => {
        const resultTitle = r?.title || r?.name || '';
        return !(0, blacklist_1.isBlacklisted)(resultTitle);
    });
