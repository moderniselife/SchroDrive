...
function cleanTorrentTitle(raw) {
    ...
    title = title.replace(/\s+(2160p|1080p|720p|480p|4K|UHD|HEVC|x265|x264|AVC|AAC|DTS|DD5|DDP5|WEB|WEBRip|WEB-DL|BluRay|BDRip|HDTV|REMUX|NF|AMZN|DSNP|HMAX|ATVP)\b.*/i, '');
 ...
 */
function extractHashFromResult(r) {
    // Direct hash fields
 ...
 */
async function tryReaddViaIndexer(rawTitle, excludeProvider, excludeHashes) {
    ...
    const results = await (0, index_1.searchIndexer)(searchTitle);
    ...
    const filteredResults = results.filter((r) => {
        ...
        if ((0, blacklist_1.isBlacklisted)(resultTitle))
            return false;
    ...
    const best = (0, index_1.pickBestResult)(filteredResults);
    let magnet = (0, index_1.getMagnet)(best);
    if (!magnet) {
        try {
            magnet = await (0, index_1.getMagnetOrResolve)(best);
        }
        catch { }
    }
    if (!magnet) {
    ...
    const providers = providers_1.registry.ordered().filter(p => p.id !== excludeProvider);
 ...
 */
async function scanProviderDead() {
    const summary = { scanned: {}, repaired: [], crossRepaired: [], readded: [], preemptive: [] };
    for (const provider of providers_1.registry.configured()) {
        try {
            ...
            const stalling = config_1.config.preemptiveRepairEnabled
                ? list.filter(t => {
                    if (provider.isTorrentDead(t))
                        return false; // Already dead, handled separately
                    if (t.progress >= 100)
                        return false; // Completed, not stalling
                    if (t.progress <= 0)
                        return false; // Not started yet, normal
                    // Check if added long ago but still not finished (stalling)
                    if (t.addedAt) {
                        const ageMin = (Date.now() - t.addedAt.getTime()) / 60000;
                        return ageMin >= config_1.config.preemptiveRepairStallMinutes;
                    }
                    return false;
                })
                : [];
            ...
            for (const t of dead) {
                ...
                if (config_1.config.enableRepair && provider.repairTorrent) {
                    let repairAttempts = 0;
                    let repaired = false;
                    while (repairAttempts < config_1.config.repairMaxAttempts && !repaired) {
                        repairAttempts++;
                        try {
                ...
                if (config_1.config.enableRepair) {
                    let infoHash = null;
                    if (provider.getInfoHash) {
                        try {
                            infoHash = await provider.getInfoHash(t.id);
                        }
                        catch { }
                    }
                    if (infoHash) {
                        const magnet = `magnet:?xt=urn:btih:${infoHash.toUpperCase()}`;
                        const otherProviders = providers_1.registry.configured().filter(p => p.id !== provider.id);
                ...
                if (provider.getInfoHash) {
                    try {
                        ...
                        if (hash)
                            excludeHashes.add(hash.toLowerCase());
                    }
                    catch { }
                }
                try {
                ...
                if (!crossRepaired) {
                    ...
                    (0, blacklist_1.addToBlacklist)(title, `Dead torrent (status: ${t.status})`, provider.id);
                    const ok = await tryReaddViaIndexer(title, provider.id, excludeHashes);
                    if (ok)
                        summary.readded.push({ from: provider.id, title });
                }
            }
        }
        catch (e) {
 ...
 */
async function scanBridgeDead() {
    const summary = { processed: 0, deleted: 0, replaced: 0 };
    const bridges = (0, mount_1.getActiveBridges)();
    for (const [providerName, bridge] of bridges) {
        const deadTorrents = bridge.getDeadTorrents();
        if (deadTorrents.size === 0)
            continue;
        console.log(`[${new Date().toISOString()}][dead-scan] bridge ${providerName} has ${deadTorrents.size} dead torrent(s)`);
        const provider = providers_1.registry.get(providerName);
        if (!provider) {
        ...
        for (const [torrentId, info] of deadTorrents) {
            ...
            (0, blacklist_1.addToBlacklist)(info.name, `Bridge-detected dead (${info.failureCount} consecutive download failures)`, providerName);
            // Also blacklist the hash itself if we have one
            for (const h of excludeHashes) {
                (0, blacklist_1.addToBlacklist)(h, `Bridge-detected dead hash (from ${info.name})`, providerName);
            }
...
function startDeadScanner() {
    ...
    const run = async () => {
        try {
            ...
            const bridgeEntries = [...(0, mount_1.getActiveBridges)().entries()];
