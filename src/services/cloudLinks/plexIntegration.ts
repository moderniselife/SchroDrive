 ...
 */
export async function triggerPlexScan(sectionIds?: number[]): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Triggering Plex scan for ${sectionIds.length} section(s): ${sectionIds.join(', ')}`
    );
    ...
    for (const id of sectionIds) {
      try {
        ...
        if (resp.ok) {
          ...
          console.warn(
            `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Scan trigger failed for section ${id}: HTTP ${resp.status}`
          );
        }
      } catch (err: any) {
        console.warn(
          `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Scan trigger error for section ${id}: ${err?.message}`
        );
      }
    }
  } catch (err: any) {
 ...
 */
async function discoverPlexSections(plexUrl: string, plexToken: string): Promise<number[]> {
  try {
    ...
    if (!resp.ok) {
      console.warn(
        `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Failed to discover sections: HTTP ${resp.status}`
      );
      return [];
    }
    ...
    console.log(
      `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Discovered ${keys.length} Plex section(s): ${keys.join(', ')}`
    );
    return keys;
  } catch (err: any) {
    console.warn(
      `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Plex discovery failed: ${err?.message}`
    );
    return [];
  }
}
 ...
 */
export async function isPlexReachable(): Promise<boolean> {
  const plexUrl = (process.env.PLEX_URL || 'http://localhost:32400').replace(/\/+$/, '');
  const plexToken = process.env.PLEX_TOKEN;
 ...
 */
export async function startPlexContainer(): Promise<boolean> {
  const containerName = process.env.PLEX_CONTAINER_NAME || 'plex';

  console.log(
    `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Pre-warm complete — starting Plex container "${containerName}"...`
  );
  ...
  try {
    ...
    if (exitCode === 0) {
      console.log(
        `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} ✅ Plex container "${containerName}" started successfully`
      );
      ...
      console.error(
        `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Failed to start Plex container (exit ${exitCode}): ${stderr.trim()}`
      );
      return false;
    }
  } catch (err: any) {
    console.error(
      `[${new Date().toISOString()}]${PLEX_LOG_PREFIX} Error starting Plex container: ${err?.message}`
    );
    return false;
  }
}
