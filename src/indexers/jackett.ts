...
import { buildMagnetFromHash, normaliseBaseUrl, stripTmdbQuery } from "./shared";
...
function normalizeResult(r: JackettResult): JackettResult {
  return {
    ...r,
...
export async function searchJackett(query: string, opts?: {
  categories?: string[];
  indexerIds?: string[];
  limit?: number;
  ...
  const categories = opts?.categories?.length ? opts.categories : (config.jackettCategories?.length ? config.jackettCategories : undefined);
  if (categories?.length) {
    params.Category = categories;
  }

  const maskedKey = config.jackettApiKey ? `${config.jackettApiKey.slice(0, 4)}…` : "unset";
  const started = Date.now();
  ...
  const limit = opts?.limit ?? (Number.isFinite(config.jackettSearchLimit) ? config.jackettSearchLimit : undefined);
  ...
  console.log(`[${new Date().toISOString()}][jackett] response`, {
    count: data.length,
    ms: Date.now() - started,
    sample: data.slice(0, 5).map((r: JackettResult) => ({
      title: r.title,
      seeders: r.seeders,
      size: r.size,
  ...
  if (!data.length && /\b(19|20)\d{2}\b/.test(usedQuery)) {
    ...
    if (withoutYear && withoutYear !== usedQuery) {
      ...
      if (!Array.isArray(rawData2)) rawData2 = [];
      ...
      console.log(`[${new Date().toISOString()}][jackett] fallback response`, {
        ...
        sample: data.slice(0, 5).map((r: JackettResult) => ({ title: r.title, seeders: r.seeders, size: r.size, indexer: r.indexer })),
      });
    }
  }
  ...
  if (!data.length && categories?.length) {
    ...
    if (!Array.isArray(rawData3)) rawData3 = [];
...
export function pickBestResult(results: JackettResult[]): JackettResult | undefined {
  // Filter out dead torrents (0 seeders) — they'll never download
  const alive = results.filter((r: JackettResult) => (r.seeders ?? 0) > 0);
...
export function getMagnet(r: JackettResult | undefined): string | undefined {
...
export async function getMagnetOrResolve(r: JackettResult | undefined): Promise<string | undefined> {
  if (!r) return undefined;
  ...
  const maxHops = Math.max(1, Math.min(Number(config.jackettRedirectMaxHops || 5), 10));
  let hops = 0;
