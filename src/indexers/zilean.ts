...
/** Shape of a single entry returned by the Zilean /dmm/filtered endpoint. */
interface ZileanEntry {
  raw_title: string;
  info_hash: string;
  size?: number;
  [key: string]: any;
 ...
 */
export async function searchZilean(query: string): Promise<ScraperResult[]> {
  ...
  const base = config.zileanUrl.replace(/\/+$/, "");
  ...
  try {
    const res = await axios.get<ZileanEntry[]>(url, {
      params: { query },
      timeout: TIMEOUT_MS,
    });
    ...
    console.log(
      `[${new Date().toISOString()}][${SOURCE}] parsed ${results.length} results from ${entries.length} entries`,
    );
