...
function getBaseUrl(): string {
  return (config.putioApiBase || 'https://api.put.io/v2').replace(/\/$/, '');
}
...
function authHeaders(overrideToken?: string): Record<string, string> {
  return {
    Authorization: `Bearer ${overrideToken || config.putioOauthToken}`,
  };
}
 ...
 */
export class PutioProvider implements DebridProvider {
  readonly id = 'putio' as const;
  readonly displayName = 'Put.io';
  ...
  async listTorrents(): Promise<TorrentInfo[]> {
    ...
    try {
      ...
      rateLimiter.setCache(TORRENT_LIST_CACHE_KEY, transfers);
  ...
  async getInfoHash(torrentId: string): Promise<string | null> {
    ...
    if (cached) {
      ...
      if (transfer?.hashString) return transfer.hashString;
    }
  ...
  async fetchDirectories(): Promise<VirtualDirectory[]> {
    ...
    try {
      ...
      const transfers: any[] = Array.isArray(data?.transfers) ? data.transfers : [];
  ...
  async resolveDownloadUrl(torrentId: string, fileId: string, _linkIndex?: number): Promise<string | null> {
    const downloadToken = tokenRotator.getDownloadToken(PROVIDER_NAME) || config.putioOauthToken;
    const isRotated = downloadToken !== config.putioOauthToken;
    ...
    try {
      ...
      if ((status === 503 || status === 429) && downloadToken !== config.putioOauthToken) {
  ...
  hasDirectWebDAV(): boolean {
    return !!(config.putioWebdavUrl && config.putioWebdavUsername && config.putioWebdavPassword);
  }
  ...
  getWebDAVConfig(): { url: string; username: string; password: string } | null {
    ...
    return {
      url: config.putioWebdavUrl,
      username: config.putioWebdavUsername,
      password: config.putioWebdavPassword,
    };
  }
  ...
  getBridgePort(): number {
    return config.webdavBridgePortPUTIO;
  }
   ...
   */
  private async fetchFilesForFolder(folderId: string): Promise<VirtualFile[]> {
    await rateLimiter.throttle(PROVIDER_NAME);
   ...
   */
  private normaliseTorrents(rawTransfers: any[]): TorrentInfo[] {
  ...
  private handleError(err: any, operation: string, overrideToken?: string): void {
    ...
    if (status === 429 || status === 503 || rateLimiter.isRateLimitError(err)) {
      if (overrideToken && overrideToken !== config.putioOauthToken) {
...
registry.register(new PutioProvider());

// Register with token rotator for download token cycling
tokenRotator.registerProvider(PROVIDER_NAME, config.putioOauthToken, config.putioDownloadTokens);
