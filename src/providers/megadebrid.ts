...
function getBaseUrl(): string {
  return (config.megadebridApiBase || 'https://www.mega-debrid.eu').replace(/\/$/, '');
}
...
function authParams(overrideToken?: string): Record<string, string> {
  return {
    token: overrideToken || config.megadebridApiKey,
  };
}
 ...
 */
export class MegaDebridProvider implements DebridProvider {
  readonly id = 'megadebrid' as const;
  readonly displayName = 'MegaDebrid';
  ...
  isConfigured(): boolean {
    return !!config.megadebridApiKey;
  }
  ...
  async fetchDirectories(): Promise<VirtualDirectory[]> {
    ...
    try {
      ...
      const torrents: any[] = Array.isArray(data) ? data : (Array.isArray(data?.torrents) ? data.torrents : []);
  ...
  async resolveDownloadUrl(torrentId: string, fileId: string, _linkIndex?: number): Promise<string | null> {
    const downloadToken = tokenRotator.getDownloadToken(PROVIDER_NAME) || config.megadebridApiKey;
    const isRotated = downloadToken !== config.megadebridApiKey;
    ...
    try {
      ...
      if ((status === 503 || status === 429) && downloadToken !== config.megadebridApiKey) {
  ...
  getBridgePort(): number {
    return config.webdavBridgePortMD;
  }
  ...
  private handleError(err: any, operation: string, overrideToken?: string): void {
    ...
    if (status === 429 || status === 503 || rateLimiter.isRateLimitError(err)) {
      if (overrideToken && overrideToken !== config.megadebridApiKey) {
...
registry.register(new MegaDebridProvider());

// Register with token rotator for download token cycling
tokenRotator.registerProvider(PROVIDER_NAME, config.megadebridApiKey, config.megadebridDownloadTokens);
