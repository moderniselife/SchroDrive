...
| Torrentio added to existing Prowlarr from Riven settings | Proposed, read-only; catalog blocked | [Riven/Arr settings matrix](cinecircle-riven-arr-settings-matrix-2026-09-17.md); `/home/samtruman/docker/cinecircle/riven/data/settings.json` (`scraping.torrentio`) | Verify Prowlarr container/config-volume persistence and preserve all existing indexers including Mircrew; approve adapter and map URL/filter/categories/language/quality before catalog construction |
