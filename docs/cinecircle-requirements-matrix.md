...
| C. AllDebrid direct/manual intake → SchröDrive → Arr | Fork scope; implemented in test, not upstream PR material | [architecture](cinecircle-pipeline-architecture-2026-09-17.md); [E2E plan](cinecircle-three-input-e2e-plan.md); `src/services/cinecircleAlldebridIntake.ts`; `tests/e2e/cinecircle-alldebrid-intake.test.ts` | Test-only adapter is not wired/deployed; production authorization remains blocked |
