...
| Torrentio configured in Prowlarr from Riven settings | Proposed, read-only; catalog blocked | [Riven/Arr settings matrix](cinecircle-riven-arr-settings-matrix-2026-09-17.md); `/home/samtruman/docker/cinecircle/riven/data/settings.json` (`scraping.torrentio`) | Approve the installed Prowlarr Torrentio adapter, map URL/filter/categories/language/quality fields, and configure it operationally before catalog construction |
